...
class MossAudioTokenizerModel(MossAudioTokenizerPreTrainedModel):
    ...
    def decode(  # type: ignore[override]
        ...
        if padding_mask is not None:
            ...
            codes_lengths = torch.full((B,), T, device=device, dtype=torch.int32)

        if chunk_duration is None:
            print(f"audio_codes: {audio_codes} ---- dtype: {audio_codes.dtype}")
            print(f"codes_lengths: {codes_lengths} ---- dtype: {codes_lengths.dtype}")
